...
// SkillName returns the flat, filesystem-safe skill name derived from a base
// name (already sanitized) and the relative path within the source.
//
...
func SkillName(baseName, relPath string) string {
	if relPath == "" {
		return baseName
	}
